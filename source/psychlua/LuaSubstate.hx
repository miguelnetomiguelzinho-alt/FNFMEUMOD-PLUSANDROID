...
{
	public static var instance:LuaSubstate;

	public var substateName:String;
	public var luaArray:Array<FunkinLua> = [];
	public var parentState:Dynamic;
	public var pauseGame:Bool = false;
	var _fallbackSubstate:FlxSubState;

	public function new(name:String, ?pauseGame:Bool = false, ?parentState:Dynamic, ?fallbackSubstate:FlxSubState)
	{
		super();
		substateName = name;
		this.pauseGame = pauseGame;
		this.parentState = parentState != null ? parentState : MusicBeatState.getState();
		_fallbackSubstate = fallbackSubstate;
	}

	public static function findScript(name:String):Null<String>
	{
		var rel:String = 'scripts/substates/$name.lua';
	...
	{
		var rel:String = 'scripts/substates/_preset.lua';
		#if sys
		#if MODS_ALLOWED
		var modded:String = Paths.modFolders(rel);
		if(FileSystem.exists(modded)) return modded;
	...
	override function create():Void
	{
		...
		if(path == null)
		{
			...
			{
				close();
				if(parentState != null && parentState.openSubState != null)
					parentState.openSubState(_fallbackSubstate);
			}
			return;
		}
	...
	function makeContext(scriptPath:String):LuaHostContext
		return new LuaHostContext(LuaHostKind.SUBSTATE, substateName, this, parentState, variables, luaArray);
	...
	function loadLua(path:String, autoCreate:Bool = false):Void
	{
		...
		{
			FunkinLua.luaTrace('[LuaSubstate:$substateName] Failed to load $path: $e', true, false, FlxColor.RED);
		}
	}
